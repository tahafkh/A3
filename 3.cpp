...
grade_table read_grade_info(string grades_info_file_name){
    ...
    while(getline(grades,grade_info)){
        ...
        index=find_course(grades_info,new_grade.id);
        if(index>=0){
...
float update_gpa(float former_gpa,int former_units,int new_units){
    float new_gpa=((21*former_gpa*(new_units-former_units))/20+former_gpa*former_units)/new_units;
    if(new_gpa>MAX_GRADE){
        new_gpa=MAX_GRADE;
    }
    return new_gpa;
}
float update_courses_info(grade_table &grades_info,const course_table &recommended_courses){
    int index;
    for(int i=0;i<recommended_courses.size();i++){
        Grade new_grade;
        new_grade.id=recommended_courses[i].id;
        new_grade.grade_no=MIN_GRADE;
        index=find_course(grades_info,recommended_courses[i].id);
        if(index>=0){
            grades_info[index].grade_no=new_grade.grade_no;
        }
        else{
...
void write_to_file(const course_table &recommended_courses,int counter){
    string recommended_courses_file_name="semester"+to_string(counter)+".sched";
    ofstream semester_courses(recommended_courses_file_name);
    for(int i=0;i<recommended_courses.size();i++){
        semester_courses<<recommended_courses[i].id<<endl;
    }
    semester_courses.close();
}
void find_recommended_courses_for_each_semester(const course_table &courses_info,grade_table &grades_info){
    int counter=1,former_units=0,new_units=0;
    float gpa=compute_gpa(courses_info,grades_info);
    course_table tackable_courses_per_semester,recommended_courses_per_semester;
    while((tackable_courses_per_semester=find_tackable_courses(courses_info,grades_info)).size() != 0){
        former_units=compute_total_units(courses_info,grades_info);
        first_sort_courses(tackable_courses_per_semester);
        recommended_courses_per_semester=find_recommended_courses(tackable_courses_per_semester,gpa);
        final_sort_courses(recommended_courses_per_semester);
        write_to_file(recommended_courses_per_semester,counter);
        counter++;
        update_courses_info(grades_info,recommended_courses_per_semester);
        new_units=compute_total_units(courses_info,grades_info);
        gpa=update_gpa(gpa,former_units,new_units);
    }
}
int main(int argc,char* argv[]){
    course_table courses=read_course_info(argv[1]);
    grade_table grades=read_grade_info(argv[2]);
    find_recommended_courses_for_each_semester(courses,grades);
    return 0;
}